...
def start_scanner(plugins, max_rate, ip_range):
    ...
    command = "./masscan/bin/masscan {} -p{} --excludefile ./blacklist.txt --max-rate {}".format(
        ip_range, ports, max_rate)
...
def entrypoint():
    ...
    parser.add_argument(
        ...
        action="store_true")
    parser.add_argument("-d", "--dest", type=ip_address,
                        help="Test with a given IP address")
    parser.add_argument(
        "-p",
        "--plugin",
        type=str,
        help="Use a specific plugin")
    parser.add_argument(
        '-v',
        '--verbose',
        action='count',
    ...
    parser.add_argument(
        ...
        type=int,
        default=150,
        help="maximum packets per seconds")
    parser.add_argument(
        '-i',
        '--ip-range',
        type=str,
    ...
    log.print_config(plugins_map, args, threads, max_rate)
