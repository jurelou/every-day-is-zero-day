...
def checkUsername(ip, username, tried=0):
    ...
    try:
        ...
        log.debug('SSH connection {} -> {}'.format(ip, e))
    try:
        ...
        log.info("plugin=openssh-15473 error=???")
    return


paramiko.auth_handler.AuthHandler._client_handler_table[
    paramiko.common.MSG_SERVICE_ACCEPT] = malform_packet
paramiko.auth_handler.AuthHandler._client_handler_table[
    paramiko.common.MSG_USERAUTH_FAILURE] = call_error
...
class Plugin(IPlugin):
    ...
    def exec(self, socket):
        ...
        if res and res[1]:
            log.info(
                "plugin=openssh-15473 device={} username={} valid={}".format(
                    socket.getpeername()[0], res[0], res[1]))
